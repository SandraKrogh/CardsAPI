...
namespace BackendAPI.Services
{
    public class MongoService
    {
        ...
        /*
                    public MongoService(CardsDatabaseSettings cardsDatabaseSettings)
                {
                    /
                    _client = new MongoClient(cardsDatabaseSettings.ConnectionString);

                    var db = _client.GetDatabase(cardsDatabaseSettings.DatabaseName);
                    if (_client.GetDatabase(cardsDatabaseSettings.DatabaseName).ListCollections().ToList().Count == 0)
                    {
                        var collectionCard = db.GetCollection<Card>(cardsDatabaseSettings.CollectionName[0]);
                        foreach (var path in new[] { "cards.json" })
                        {
                            using (var file = new StreamReader(path))
                            {
                        ...
                        var collectionSet = db.GetCollection<Set>(cardsDatabaseSettings.CollectionName[4]);
                        foreach (var path in new[] { "sets.json" })
                        {
                            using (var file = new StreamReader(path))
                            {
                        ...
                        var collectionRarity = db.GetCollection<Rarity>(cardsDatabaseSettings.CollectionName[3]);
                        foreach (var path in new[] { "rarities.json" })
                        {
                            using (var file = new StreamReader(path))
                            {
                        ...
                        var collectionTypes = db.GetCollection<CardType>(cardsDatabaseSettings.CollectionName[1]);
                        foreach (var path in new[] { "types.json" })
                        {
                            using (var file = new StreamReader(path))
                            {
                        ...
                        var collectionClass = db.GetCollection<Class>(cardsDatabaseSettings.CollectionName[2]);
                        foreach (var path in new[] { "classes.json" })
                        {
                            using (var file = new StreamReader(path))
                            {
